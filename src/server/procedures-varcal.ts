...
import { AliasDefEst } from "./types-procesamiento";

export interface coreFunctionParameters{
    operativo: string
}

export type CoreFunction = (context: operativos.ProcedureContext, parameters: coreFunctionParameters) => Promise<Procesamiento.DefinicionEstructural>;
...
var ProceduresProcesamiento = [
    {
        ...
        coreFunction: async function(context:operativos.ProcedureContext, parameters: coreFunctionParameters){
            ...
            let defEst: Procesamiento.DefinicionEstructural ={
                aliases: {},
                tables: {}
            }
            ...
            results.tables.rows.forEach(function(table: UnidadDeAnalisis & {pk_arr: string[]}){
                ...
                let tDefEst:Procesamiento.DefinicionEstructuralTabla = {
                    sourceBro : tua,
                    target: tua + Procesamiento.sufijo_tabla_calculada,
                    pks : table.pk_arr,
                    aliasAgg : tua + Procesamiento.sufijo_agregacion,
                }
                tDefEst.where = Procesamiento.generateConditions(tDefEst.sourceBro, tDefEst.target, table.pk_arr);
              
                tDefEst.whereAgg = {};
                tDefEst.sourceJoin = '';
                if (table.padre){
                    tDefEst.sourceAgg = tDefEst.target + ` inner join ${tua} ON ` + Procesamiento.generateConditions(tDefEst.target, tua, table.pk_arr);
                    ...
                    // Calculo whereAgg
                    tDefEst.whereAgg[table.padre] = Procesamiento.generateConditions(table.padre, tDefEst.target, pksPadre);
                    // Calculo sourceJoin
    ...
    },
    {
        ...
        coreFunction: async function (context: operativos.ProcedureContext, parameters: coreFunctionParameters) {
            ...
            var estructuraParaGenerar: Procesamiento.DefinicionEstructural = await (be.procedure['definicion_estructural/armar'].coreFunction(context, {operativo}) as Promise<Procesamiento.DefinicionEstructural>)

            resultUA.rows.forEach(function (row) {
                var estParaGenTabla:Procesamiento.DefinicionEstructuralTabla = estructuraParaGenerar.tables[row.unidad_analisis];
            ...
            function wrapExpression(expression:string|number, pkExpression:string) {
                var opts:Procesamiento.CompilerOptions = { language: 'sql', varWrapper: 'null2zero', divWrapper: 'div0err', elseWrapper: 'lanzar_error' };
            ...
            `, [operativo]).fetchAll();
            var allVariables: Procesamiento.VariablesDefinidas = {};
            ...
            var grupoVariables = Procesamiento.separarEnGruposPorNivelYOrigen(variablesACalcular, Object.keys(likear(allVariables).filter(v => v.clase != 'calculada')), estructuraParaGenerar);
            var parametrosGeneracion = {
                nombreFuncionGeneradora: 'gen_fun_procesamientoc',
                esquema: be.config.db.schema,
            };
            var funcionGeneradora = Procesamiento.funcionGeneradora(grupoVariables, parametrosGeneracion, estructuraParaGenerar, allVariables);
